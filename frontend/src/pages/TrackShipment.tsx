...
const TrackShipment: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (trackingShipment && socket) {
      socket.emit('track:join', trackingShipment.trackingNumber);

      const handleUpdate = (data: any) => {
        toast.success(`Shipment status updated to ${data.status}`);
        dispatch(trackShipment(trackingShipment.trackingNumber));
      };

      socket.on('tracking:update', handleUpdate);
      socket.on('error', (err: any) => {
        toast.error(err.message || 'Tracking socket error');
      });

      return () => {
        socket.emit('track:leave', trackingShipment.trackingNumber);
        socket.off('tracking:update', handleUpdate);
        socket.off('error');
      };
    }
  }, [trackingShipment, dispatch]);
