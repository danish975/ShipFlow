    ...
    const shipment = await Shipment.create({
      ...
      statusHistory: [
        ...
        },
        {
          ...
          timestamp: new Date(),
          updatedBy: req.user.userId,
          reason: 'Auto-validated upon creation',
        }
      ],
...
export const updateStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const isAdmin = req.user.role === UserRole.ADMIN || req.user.role === 'operations_manager';
    ...
    if (status === ShipmentStatus.PICKED_UP) {
      ...
      const { recipientName, otp } = req.body;
      const { DeliveryProof } = await import('../models/DeliveryProof');
      
      const existingProof = await DeliveryProof.findOne({ shipmentId: shipment._id });
      if (!existingProof && recipientName) {
...
export const assignAgent = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (shipment.status !== ShipmentStatus.CREATED && shipment.status !== ShipmentStatus.VALIDATED) {
      throw ApiError.badRequest('Can only assign agent to newly created/validated shipments', 'INVALID_ASSIGNMENT');
    }
